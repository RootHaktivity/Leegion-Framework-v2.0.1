...
class VPNManager(BaseModule):
    ...
    def _display_vpn_menu(self):
        ...
        try:
            ...
            print_clean_menu_header(
                "VPN MANAGER", "Secure VPN Connection Management"
            )
            print(
    ...
    def _connect_vpn(self):
        ...
        display_choice = (
            self.get_user_input(
                "\nSelect display mode (1-3): ", required=False
            ) 
            or "1"
        )
        ...
        if display_choice in ["1", "2", "3"]:
            self.print_info(
                f"Starting VPN connection to: {selected_config['name']}"
            )
    ...
    def _start_vpn_connection(
        self, config: Dict[str, str], display_mode: str = "1"
    ):
        ...
        def run_vpn():
            try:
                ...
                if not os.path.exists(config["path"]):
                    self.print_error(
                        f"Configuration file not found: {config['path']}"
                    )
                    return
                ...
                if config.get("username") and config.get("password"):
                    cmd.extend([
                        "--auth-user-pass", config["auth_file"]
                    ])
    ...
    def _update_simple_vpn_display(
        ...
        print(
            f"Established: {'Yes' if connection_established else 'No'}"
        )
    ...
    def _show_connection_status(self):
        ...
        try:
            ...
            if status["connected"]:
                print(
                    f"\033[96mConfiguration:\033[0m {status.get('config', 'N/A')}"
                )
                print(f"\033[96mUptime:\033[0m {status.get('uptime', 'N/A')}")
                print(f"\033[96mProcess ID:\033[0m {status.get('pid', 'N/A')}")
                print(
                    f"\033[96mVPN Interface:\033[0m {status.get('interface', 'N/A')}"
                )
                ...
                if ip_info:
                    print(
                        f"\033[96mPublic IP:\033[0m {ip_info.get('ip', 'Unknown')}"
                    )
                    print(
                    ...
                    print(
                        "\033[91mCould not retrieve public IP information\033[0m"
                    )
            else:
                print(
    ...
    def _list_vpn_configs(self):
        ...
        for i, config in enumerate(configs, 1):
            ...
            print(
                f"   Modified: {self._get_file_modified_date(config['path'])}"
            )
    ...
    def _show_connection_stats(self):
        ...
        print(f"\033[96mTotal Uptime:\033[0m {self._format_uptime(stats['total_uptime'])}")
        
        if stats['external_vpn_detected']:
            print("\033[96mExternal VPN:\033[0m Active")
            if stats['external_vpn_start_time']:
                external_uptime = (
                    time.time() - stats['external_vpn_start_time']
                )
                print(
    ...
    def _import_vpn_config(self):
        ...
        if not config_path.endswith(('.ovpn', '.conf')):
            self.print_error(
                "Unsupported file format. Use .ovpn or .conf files."
            )
            return
    ...
    def _test_vpn_config(self):
        ...
        selected_config = configs[choice_idx]
        self.print_info(
            f"Testing configuration: {selected_config['name']}"
        )
